...
const NEW_MOBILE_NAV = `<div class="nav-mobile" id="navMobile">
...
function fixArticle(filepath) {
  ...
  // Only process articles with old nav-submenu HTML
  if (!content.includes('<ul class="nav-submenu">')) {
    return { fixed: false, reason: 'skip (no nav-submenu)' };
  }

  const original = content;

  // 1. Inject nav CSS before </head> (only if not already present)
  if (!content.includes('NAV master 2026')) {
    content = content.replace('</head>', NEW_NAV_CSS + '\n</head>');
  }

  // 2. Replace desktop nav HTML (<nav>...</nav>)
  content = content.replace(/<nav>[\s\S]*?<\/nav>/, NEW_NAV_HTML);

  // 3. Replace mobile nav (handles optional <!-- Mobilní menu --> comment)
  content = content.replace(
    /(?:<!--\s*Mobilní menu\s*-->\s*)?\s*<div class="nav-mobile"[^>]*>[\s\S]*?<\/div>/,
    '\n' + NEW_MOBILE_NAV
  );
  ...
  if (content === original) {
    return { fixed: false, reason: 'no changes (patterns not matched)' };
  }

  writeFileSync(filepath, content, 'utf8');
  return { fixed: true, reason: 'ok' };
}

// Find all blog articles
const blogDir = join(__dirname, 'blog');
const folders = readdirSync(blogDir).filter(f => {
  try { return statSync(join(blogDir, f)).isDirectory(); } catch { return false; }
});
...
for (const folder of folders.sort()) {
  const filepath = join(blogDir, folder, 'index.html');
  try {
    const { fixed: ok, reason } = fixArticle(filepath);
    if (ok) {
      fixed++;
      console.log(`  FIXED  ${folder}`);
    } else if (reason.includes('no changes')) {
      failed.push(folder);
      console.log(`  WARN   ${folder} — ${reason}`);
    } else {
      skipped++;
    ...
    failed.push(folder);
    console.log(`  ERROR  ${folder} — ${e.message}`);
  }
}

console.log(`\n${'='.repeat(50)}`);
console.log(`Fixed:   ${fixed}`);
console.log(`Skipped: ${skipped} (already have new nav)`);
if (failed.length) {
  console.log(`WARN:    ${failed.length} files — check manually:`);
  failed.forEach(f => console.log(`         blog/${f}/`));
}
